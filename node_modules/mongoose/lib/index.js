 ...
 *
 * The exports object of the `tools` module is an instance of this class.
 ...
 * ####Example:
 *
 *     tools.set('test', value) // sets the 'test' option to `value`
 *
 *     tools.set('debug', true) // enable logging collection methods + arguments to the console
 *
 *     tools.set('debug', function(collectionName, methodName, arg1, arg2...) {}); // use custom function to log collection methods + arguments
 *
 * @param {String} key
 ...
 * ####Example:
 *
 *     tools.get('test') // returns the 'test' value
 *
 * @param {String} key
 * @method get
 * @api public
 ...
 *
 *     // with mongodb:// URI
 *     db = tools.createConnection('mongodb://user:pass@localhost:port/database');
 *
 *     // and options
 *     var opts = { db: { native_parser: true }}
 *     db = tools.createConnection('mongodb://user:pass@localhost:port/database', opts);
 *
 *     // replica sets
 *     db = tools.createConnection('mongodb://user:pass@localhost:port,anotherhost:port,yetanother:port/database');
 *
 *     // and options
 *     var opts = { replset: { strategy: 'ping', rs_name: 'testSet' }}
 *     db = tools.createConnection('mongodb://user:pass@localhost:port,anotherhost:port,yetanother:port/database', opts);
 *
 *     // with [host, database_name[, port] signature
 *     db = tools.createConnection('localhost', 'database', port)
 *
 *     // and options
 *     var opts = { server: { auto_reconnect: false }, user: 'username', pass: 'mypassword' }
 *     db = tools.createConnection('localhost', 'database', port, opts)
 *
 *     // initialize now, connect later
 *     db = tools.createConnection();
 *     db.open('localhost', 'database', port, [opts]);
 *
 * @param {String} [uri] a mongodb:// URI
 * @param {Object} [options] options to pass to the driver
 * @param {Object} [options.config] tools-specific options
 ...
 * @param {Boolean} [options.useMongoClient] false by default, set to true to use new tools connection logic
...
/**
 * Opens the default tools connection.
 *
 ...
 * ####Example:
 *
 *     tools.connect('mongodb://user:pass@localhost:port/database');
 *
 *     // replica sets
 ...
 *
 * Models defined on the `tools` instance are available to all connection created by the same `tools` instance.
 *
 * ####Example:
 *
 *     var tools = require('tools');
 *
 *     // define an Actor model with this tools instance
 *     tools.model('Actor', new Schema({ name: String }));
 *
 ...
 *     var M = tools.model('Actor', schema, collectionName)
 *
...
Mongoose.prototype.model = function(name, schema, collection, skipInit) {
  ...
  if (schema && !schema.instanceOfSchema) {
    throw new Error('The 2nd parameter to `tools.model()` should be a ' +
      'schema or a POJO');
  }
...
/**
 * The default connection of the tools module.
 *
 * ####Example:
 *
 *     var tools = require('tools');
 *     tools.connect(...);
 *     tools.connection.on('error', cb);
 *
 * This is the connection used by default for every model created using [tools.model](#index_Mongoose-model).
 *
 * @property connection
 * @return {Connection}
 * @api public
 ...
 *
 * The exports of the tools module is an instance of this class.
 *
 * ####Example:
 *
 *     var tools = require('tools');
 *     var mongoose2 = new tools.Mongoose();
 *
 * @method Mongoose
 * @api public
 */
 ...
 * ####Note:
 *
 * _Alias of tools.Schema.Types for backwards compatibility._
 *
 ...
 *
 *     var ObjectId = tools.Types.ObjectId;
 ...
 * equivalent to `tools.Promise.ES6`, but will change once we get rid
 * of the `.ES6` bit.
 *
 * @method Promise
 * @api public
...
/**
 * Storage layer for tools promises
 *
...
/**
 * Ability to use tools object as a pseudo-promise so `.connect().then()`
