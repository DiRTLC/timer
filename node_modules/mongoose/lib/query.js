 ...
 *
 *     tools.Query.use$geoWithin = false;
 *
 ...
 *
 *     // Adventure is now a subclass of tools.Query and works the same way but with the
 ...
 *       docs[0] instanceof tools.Document // false
 *     });
 *
...
/**
 * Getter/setter around the current tools-specific options for this query
 * (populate, lean, etc.)
 *
 ...
 * @param {Boolean} [options.multipleCastError] by default, tools only returns the first error that occurred in casting the query. Turn on this option to aggregate all the cast errors.
...
/*!
 * Override mquery.prototype._mergeUpdate to handle tools objects in
 * updates.
 *
 * @param {Object} doc
 * @api private
 ...
 *       // do something with the tools document
 ...
 * ####Valid options
 *
 *   - `transform`: optional function which accepts a tools document. The return value of the function will be emitted on `data`.
 *
 * ####Example
 *
...
Query.prototype.stream = util.deprecate(Query.prototype.stream, 'Mongoose: ' +
  'Query.prototype.stream() is deprecated in tools >= 4.5.0, ' +
 ...
 * ####Valid options
 *
 *   - `transform`: optional function which accepts a tools document. The return value of the function will be emitted on `data` and returned by `.next()`.
 *
 * @return {QueryCursor}
 * @param {Object} [options]
 * @see QueryCursor
...
/*!
 * this is needed to support the tools syntax of:
