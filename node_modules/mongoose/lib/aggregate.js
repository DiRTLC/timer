 ...
 * - Mongoose does **not** cast pipeline stages. `new Aggregate({ $match: { _id: '00000000000000000000000a' } });` will not work unless `_id` is a string in the database. Use `new Aggregate({ $match: { _id: tools.Types.ObjectId('00000000000000000000000a') } });` instead.
 *
 ...
 * @param {Boolean} [options.useMongooseAggCursor] use experimental tools-specific aggregation cursor (for `eachAsync()` and other query cursor semantics)
