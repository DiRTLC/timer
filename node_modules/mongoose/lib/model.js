...
function Model(doc, fields, skipId) {
  if (fields instanceof Schema) {
    throw new TypeError('2nd argument to `Model` must be a POJO or string, ' +
      '**not** a schema. Make sure you\'re calling `tools.model()`, not ' +
      '`tools.Model()`.');
  }
...
Model.prototype.$__handleSave = function(options, callback) {
  ...
  if (this.isNew) {
    ...
    if ((obj || {})._id === void 0) {
      // documents must have an _id else tools won't know
 ...
 * 3. `numAffected` will be 1 when the document was successfully persisted to MongoDB, otherwise 0. Unless you tweak tools's internals, you don't need to worry about checking this parameter for errors - checking `err` is sufficient to make sure your document was properly saved.
 *
 ...
 *     });
 *
 * For legacy reasons, tools stores object keys in reverse order on initial
 ...
 *
 *     var Person = tools.model('Person', PersonSchema);
 ...
 *     var Event = tools.model('Event', eventSchema);
 *
 ...
 * to `findOne({})` and return arbitrary documents. However, tools
 ...
 *
 *     // passing options (in this case return the raw js objects, not tools documents by passing `lean`
 ...
 * tools will send an empty `findOne` command to MongoDB, which will return
 ...
 *
 * Sometimes you need to query for things in mongodb using a JavaScript expression. You can do so via `find({ $where: javascript })`, or you can use the tools shortcut method $where via a Query chain or from your tools Model.
 *
 ...
 * - `setDefaultsOnInsert`: if this and `upsert` are true, tools will apply the [defaults](http://mongoosejs.com/docs/defaults.html) specified in the model's schema if a new document is created. This option only works on MongoDB >= 2.4 because it relies on [MongoDB's `$setOnInsert` operator](https://docs.mongodb.org/v2.4/reference/operator/update/setOnInsert/).
 ...
 * @param {Boolean} [options.rawResult = false] if false, the returned promise resolves to the documents that passed tools document validation. If `false`, will return the [raw result from the MongoDB driver](http://mongodb.github.io/node-mongodb-native/2.2/api/Collection.html#~insertWriteOpCallback) with a `tools` property that contains `validationErrors` if this is an unordered `insertMany`.
 * @param {Function} [callback] callback
 * @return {Promise}
 * @api public
 */
...
Model.insertMany = function(arr, options, callback) {
  ...
  parallelLimit(toExecute, limit, function(error, docs) {
    ...
    _this.collection.insertMany(docObjects, options, function(error, res) {
      ...
      if (rawResult) {
        if (ordered === false) {
          // Decorate with tools validation errors in case of unordered,
 ...
 *           // `update: { $set: { title: ... } }` but tools adds $set for
 *           // you.
 ...
 * - The documents returned are plain javascript objects, not tools documents (since any shape of document can be returned).
...
function populate(model, docs, options, callback) {
  ...
  for (i = 0; i < len; i++) {
    ...
    // field is not selected, automatically select it because tools needs it.
 ...
 * @param {Mongoose} base tools instance
 */
